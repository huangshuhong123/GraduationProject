...
public class BaseUtil {
     ...
     * 拆分后的集合为[1-, 1-2-, 1-2-3-]
     */
    public static List<String> convertFullNameList(String fullName) {
        List<String> list = new ArrayList<>();
        if(BaseUtil.isNotEmpty(fullName)){
            String[] fullNames = fullName.split(DEPT_APPEND);
            if(BaseUtil.isNotEmpty(fullNames) && fullNames.length > 0){
                StringBuilder fullNameItem = new StringBuilder();
                for (int i = 0;i< fullNames.length; i++) {
                    fullNameItem.append(fullNames[i]).append(DEPT_APPEND);
                    list.add(fullNameItem.toString());
                }
            }
        }
        return list;
